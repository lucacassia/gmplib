...
$$\rho^{(N,0)}_{u_0,\dots,u_{N-1}}(x^{+}_0) \cdot P_{\boldsymbol{\lambda}} = \mathrm{eigenvalue}(\boldsymbol{\lambda})\cdot P_{\boldsymbol{\lambda}}, \qquad \mathrm{eigenvalue}(\boldsymbol{\lambda}) = \sum_{i=0}^{N-1} u_i \, x_{2d}(\lambda^{(i)})$$
